...
off_t offset;
struct c_array //for -c option. make struct.
{
	char buf[BUFSIZE];
	int cnt;
	bool check;
...
void ssu_history(int argc, char *argv[])
{
	...
	for(i=1; i<argc; i++) //check flag. to take -c -r -n option.
	{
	...
	printf("num =%d\n", num);
#endif
	if((flag_unknown == true)||(num < 0)||(num>100))// if unknown flag set. than print error.
	{
	...
	if(flag_c == true)// if flag_c set. than use a struct to take that option.
	{
		...
		else
		{
			for(i=0; i<c_size; i++)
			{
				...
				{
					if(strcmp(c_array[i].buf, c_array[j].buf)>=0) //struct intialization.
					{
						strcpy(c_buf, c_array[i].buf);
						strcpy(c_array[i].buf, c_array[j].buf);
						strcpy(c_array[j].buf, c_buf);
					}
				}
			}
		}
		...
		for(i=0; i<c_size; i++)
		{
			printf("new c_array[%d] = %s\n", i, c_array[i].buf);
		}
#endif
		cnt = 1;
		index = 0;
		for(i=1 ;i<c_size ;i++) //check how many same cmd in that array.
		{
			if(strcmp(c_array[index].buf, c_array[i].buf)==0)
			{
				++cnt;
				c_array[index].cnt = cnt;
				c_array[i].cnt =0;
			}
			else
			{
		...
		for(i=0; i<c_size; i++)
		{
			printf("new c_array[%d].buf = %s, cnt = %d\n", i, c_array[i].buf, c_array[i].cnt);
		}
#endif
		for(i=0; i<c_size; i++) //check how many smae cmd in that array.
		{
			for(j=i+1; j<c_size;j++)
			{
				if(c_array[i].cnt >= c_array[j].cnt)
				{
					cnt = c_array[i].cnt;
					c_array[i].cnt = c_array[j].cnt;
					c_array[j].cnt = cnt;
					strcpy(c_buf, c_array[i].buf);
					strcpy(c_array[i].buf, c_array[j].buf);
					strcpy(c_array[j].buf, c_buf);
		...
		if(flag_r == true)
		{
			...
			for(i=c_size-1 ;i>0; i--)
			{
				...
				if(cnt > num)
				{
					c_array[i].cnt = c_array[i].cnt -(cnt -num);
					c_index = i;
					break;
				}
				else if(cnt == num)
			...
			for(i= c_index; i<c_size; i++)
			{
				printf("%s %d\n", c_array[i].buf, c_array[i].cnt);
			}
		}
		else
		{
	...
	{
		fd = open("store.txt", O_CREAT|O_RDONLY, 0644); //if flag_c doesn't set. try this paragarh
...
void ssu_grep(int argc, char *argv[])//ssu_grep function.
{
	...
	strcpy(string, argv[1]);// argv[1] must be string. so copy.
	for(i=2; i<argc; i++)// set the flag to take the option.
	{
...
#endif
	if((flag_unknown == true)||(argc < 3))//usage print
	{
		...
		cnt =0;
		{
			...
			if(S_ISDIR(file_info.st_mode))
			{
				...
				if(flag_r == true)
				{
					...
					for(;;)// to deal with option -r, I use recurive.
					{
...
int main(int argc, char *argv[])
{
	...
	while(1)
	{
		...
		if(arg[0]=='\n')// if i just push enter, continue the loop
			continue;
		...
		strcat(tmp, str[0]);
		for(i =1 ;i<arg_count; i++) /*arg를 붙일경우 abort error가 발생함*/
		{
			tmp[strlen(tmp)] = ' ';	
			//	printf("tmp = %s^\n", tmp);
			strcat(tmp, str[i]);
