...
int main(int argc, char *argv[1])
{
...
#endif
	pr_sysconf("POSIX_JOB_CONTROL = ", _SC_JOB_CONTROL);
	pr_sysconf("POSIX_SAVED_IDS = ", _SC_SAVED_IDS);
	pr_sysconf("POSIX_VERSION = ", _SC_VERSION);
	
	pr_pathconf("MAX_CANON = ", "/dev/tty", _PC_MAX_CANON);
	pr_pathconf("MAX_INPUT = ", "/dev/tty", _PC_MAX_INPUT);
	pr_pathconf("VDISABEL = ", "/dev/tty", _PC_VDISABLE);
	...
	pr_pathconf("NO_TRUNC = ", argv[1], _PC_NO_TRUNC);
