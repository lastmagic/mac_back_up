...
void ssu_history(int argc, char *argv[])
{
	...
	for(i=1; i<argc; i++)
	{
		...
		if((temp =strchr(argv[i], 'r'))!=NULL)
			flag_r =true;
		if((temp =strchr(argv[i],'n'))!=NULL&&(temp=strchr(argv[i],'c'))==NULL&&((temp=strchr(argv[i],'r'))==NULL))
		{
...
#ifdef DEBUG
	printf("ssu_grep!\n");
	printf("flag_d =%d, flag_e = %d, flag_i = %d, flag_n =%d, flag_r =	%d, flag_unknown= %d\n", flag_d,flag_e,flag_i, flag_n, flag_r, flag_unknown);
	printf("name =%s\n", name);
#endif
